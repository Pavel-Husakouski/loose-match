...
describe('Type inference: ExpressionRule', () => {
  expect<Infer<ExpressionRule<string>>>().isOfType<string>().equals<true>();
  expect<Infer<ExpressionRule<number>>>().isOfType<number>().equals<true>();
  expect<Infer<ExpressionRule<bigint>>>().isOfType<bigint>().equals<true>();
  expect<Infer<ExpressionRule<boolean>>>().isOfType<boolean>().equals<true>();
  expect<Infer<ExpressionRule<null>>>().isOfType<null>().equals<true>();
  expect<Infer<ExpressionRule<undefined>>>().isOfType<undefined>().equals<true>();
  expect<Infer<ExpressionRule<Date>>>().isOfType<Date>().equals<true>();
  expect<Infer<ExpressionRule<ExpressionRule<RegExp>>>>().isOfType<ExpressionRule<RegExp>>().equals<true>();
});
...
describe('Type inference: union and intersection', () => {
  expect<InferIntersection<[{ id: string }, { email: string }]>>()
    ...
    .equals<true>();
  expect<InferIntersection<[{ id: string }, { email: ExpressionRule<string> }]>>()
    .isOfType<{ id: string } & { email: string }>()
    .equals<true>();
});
...
describe('Type inference: complex type object', () => {
  expect<Infer<{ messages: (ExpressionRule<string> | ExpressionRule<number>)[] }>>()
    .isOfType<{ messages: (string | number)[] }>()
    .equals<true>();
});

describe('Type inference: complex type object', () => {
  expect<Infer<{ messages: (ExpressionRule<{ id: string }> | ExpressionRule<{ email: string }>)[] }>>()
...
describe('strictEqual', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<typeof globalThis>>().equals<true>();
});

describe('literal: string value', () => {
  const pattern = literal('1');

  expect(pattern).isOfType<ExpressionRule<'1'>>().equals<true>();
});

describe('literal: Date value', () => {
  const pattern = literal(new Date());

  expect(pattern).isOfType<ExpressionRule<Date>>().equals<true>();
});
...
describe('arrayOf: length', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<string[]>>().equals<true>();
});

describe('literal regexp', () => {
  const pattern = literal(/xxx/);

  expect(pattern).isOfType<ExpressionRule<RegExp>>().equals<true>();
});
...
describe('arrayOf: anyOf with mixed types', () => {
  ...
  expect<typeof pattern>().isOfType<ExpressionRule<(number | '2' | Date)[]>>().equals<true>();
  expect<typeof pattern>().isOfType<ExpressionRule<(number | string | Date)[]>>().equals<false>();
});

describe('arrayOf: anyOf with primitives and boolean', () => {
  const pattern = arrayOf(anyOf(1, 2, 's', aBoolean(), 5));

  expect(pattern).isOfType<ExpressionRule<(1 | 2 | 's' | boolean | 5)[]>>().equals<true>();
});

describe('tuple: mixed types', () => {
  const pattern = tuple([aNumber(), '2', 8, new Date()]);

  expect(pattern).isOfType<ExpressionRule<[number, '2', 8, Date]>>().equals<true>();
});
...
describe('tuple: union type', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<[number, '2', 8, Date]>>().equals<true>();
  expect(pattern).isOfType<ExpressionRule<(number | string | Date)[]>>().equals<false>();
});
...
describe('array: empty', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<never[]>>().equals<true>();
});

describe('array: mixed values', () => {
  const pattern = array([1, 2, 3, 5, 's']);

  expect(pattern).isOfType<ExpressionRule<(1 | 2 | 3 | 5 | 's')[]>>().equals<true>();
});
...
describe('arrayOf: anyOf with primitives and Date', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<('1' | 5 | Date)[]>>().equals<true>();
});

describe('objectLike: with length property', () => {
  const pattern = objectLike({ length: 5 });

  expect(pattern).isOfType<ExpressionRule<{ length: 5 }>>().equals<true>();
  expect(pattern).isOfType<ExpressionRule<{ length: number }>>().equals<false>();
  expect(pattern).isOfType<ExpressionRule<Array<any>>>().equals<false>();
});
...
describe('objectShape: string and number', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<{ a: string; b: number }>>().equals<true>();
});
...
describe('optional: string literal', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<'1' | undefined>>().equals<true>();
});

describe('nullable: string literal', () => {
  const pattern = nullable('1' as const);

  expect(pattern).isOfType<ExpressionRule<'1' | null>>().equals<true>();
  expect(pattern).isOfType<ExpressionRule<'1'>>().equals<false>();
  expect(pattern).isOfType<ExpressionRule<null>>().equals<false>();
});

describe('nullable: aString', () => {
  const pattern = nullable(aString());

  expect(pattern).isOfType<ExpressionRule<string | null>>().equals<true>();
  expect(pattern).isOfType<ExpressionRule<string>>().equals<false>();
});
...
describe('nullable: objectShape', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<{ a: string } | null>>().equals<true>();
});
...
describe('oneOf: primitives and combinators', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<string | number | Date>>().equals<true>();
});
...
describe('anyOf: object with regexp', () => {
  ...
  expect(pattern).isOfType<ExpressionRule<{ id: string } | { id: string } | { id: string }>>().equals<true>();
  expect(pattern).isOfType<ExpressionRule<{ readonly id: string }>>().equals<false>();
});

describe('anyOf: string, object, array', () => {
  const pattern = anyOf(aString(), { id: re(/x/) }, arrayOf(aString()));

  expect(pattern).isOfType<ExpressionRule<string | { id: string } | string[]>>().equals<true>();
});

describe('anyOf: string literals', () => {
  const pattern = anyOf('1', '2', '3');

  expect(pattern).isOfType<ExpressionRule<'1' | '2' | '3'>>().equals<true>();
});
...
describe('anyOf: object id union', () => {
  ...
  expect(pattern)
    .isOfType<ExpressionRule<{ readonly id: '1' } | { readonly id: '2' } | { readonly id: '4' }>>()
    .equals<true>();
});
...
describe('anyOf: object id union as union type', () => {
  ...
  expect<typeof pattern>().isOfType<ExpressionRule<{ id: '1' } | { id: '2' } | { id: '4' }>>().equals<true>();
  expect<typeof pattern>().isOfType<ExpressionRule<{ id: '1' | '2' | '5' }>>().equals<false>();
});

describe('instanceOf: Error', () => {
  const pattern = instanceOf(Error);

  expect(pattern).isOfType<ExpressionRule<Error>>().equals<true>();
});

describe('instanceOf: Error with details', () => {
  const pattern = instanceOf(Error, { details: aString() });

  expect(pattern).isOfType<ExpressionRule<Error & { details: string }>>().equals<true>();
});

describe('instanceOf: RegExp with source', () => {
  const pattern = instanceOf(RegExp, { source: 'xxx' });

  expect(pattern).isOfType<ExpressionRule<RegExp & { source: string }>>().equals<true>();
});

describe('nullish: aString', () => {
  const pattern = nullish(aString());

  expect(pattern).isOfType<ExpressionRule<string | null | undefined>>().equals<true>();
});
...
describe('predicate: function', () => {
  ...
  expect<Pattern>().isOfType<{}>().equals<true>();
});
